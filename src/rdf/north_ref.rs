...
pub struct NorthReferenceTracker {
...
impl NorthReferenceTracker {
    ...
    /// Process buffer and detect north tick
    /// Returns vec of detected north ticks with timing info
    pub fn process_buffer(&mut self, buffer: &[f32]) -> Vec<NorthTick> {
        ...
        for peak_idx in peaks {
            ...
            // Update rotation period estimate
            if let Some(last) = self.last_tick_sample {
                let period = (global_sample - last) as f32;

                // Smooth the period estimate with exponential averaging
...
mod tests {
    ...
    fn test_north_tick_detection() {
        ...
        // Create signal with pulses at ~500Hz (every 96 samples)
        let mut signal = vec![0.0; 500];
        signal[50] = 0.8;
        signal[146] = 0.8; // 96 samples later
        signal[242] = 0.8; // 96 samples later
