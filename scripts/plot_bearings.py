...
def main():
    ...
    parser.add_argument(
        "csv_file", nargs="?", help="Path to single CSV file (legacy mode)"
    )
    parser.add_argument(
        "--correlation", "-c", type=str, help="Path to correlation method CSV"
    )
    parser.add_argument(
        "--zero-crossing", "-z", type=str, help="Path to zero-crossing method CSV"
    )
    ...
    parser.add_argument(
        "--no-show", action="store_true", help="Do not display the plot (just save)"
    )
...
def plot_single(args):
    ...
    print(
        f"Plotting {len(df_filtered)}/{len(df)} points "
    ...
    ax1 = axes[0]
    ax1.scatter(
        df_filtered["time_s"], df_filtered["bearing"], s=1, alpha=0.5, label="Smoothed"
    )
...
def plot_comparison(args):
    ...
    if args.correlation:
        ...
        if len(df_corr) == 0:
            ...
            print(
                f"Correlation: {len(df_corr_f)}/{len(df_corr)} points after filtering"
            )
    ...
    if (
        df_corr_f is not None
        and df_zc_f is not None
        and len(df_corr_f) > 0
        and len(df_zc_f) > 0
    ):
        ...
        if len(merged) > 0:
            ...
            print(
                f"Method difference: mean={mean_diff:.1f}°, std={std_diff:.1f}°, "
